...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n<h4 data-id=\""
    ...
    + ".\n    <br>Did this artist write this song? "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.written_by : stack1), depth0))
    + "\n    <br>Song ID = "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\n    <form class=\"delete-song\" id=\"delete-song\">\n      <div class=\"form-group\">\n          <button type=\"submit\" class=\"btn btn-default\">Delete Song</button>\n      </div>\n    </form>\n    <button type=\"submit\" class=\"update-song\">Edit Song</button>\n</h4><br>\n\n\n\n";
...
var patchSong = function patchSong(data, songId) {
  return $.ajax({
    url: config.apiOrigin + '/songs/' + songId,
    method: 'PATCH',
    headers: {
...
var setAPIOrigin = '../../spec/lib/set-api-origin';
var userUi = __webpack_require__(16);
var config = __webpack_require__(3);
var getFormFields = __webpack_require__(9);
var userApi = __webpack_require__(8);
var store = __webpack_require__(4);
var songHandles = __webpack_require__(6);

// Song functions
var createSong = function createSong(event) {
  event.preventDefault();
  var data = getFormFields(this);
  userApi.newSong(data).then(userUi.onNewSongSuccess).then($(this).trigger('reset')).catch(userUi.onNewSongFailure);
};

var showSongs = function showSongs(event) {
  event.preventDefault();
  userApi.get().then(userUi.onGetSuccess).catch(userUi.onGetFailure);
};
...
var signUp = function signUp(event) {
  event.preventDefault();
  var data = getFormFields(this);
  userApi.createUser(data).then(userUi.onSignUpSuccess).then($(this).trigger('reset')).catch(userUi.onSignUpFailure);
};

var signIn = function signIn(event) {
  event.preventDefault();
  var data = getFormFields(this);
  userApi.logIn(data).then(userUi.onSignInSuccess).then($(this).trigger('reset')).catch(userUi.onSignInFailure);
};

var signOut = function signOut(event) {
  event.preventDefault();
  var data = getFormFields(this);
  userApi.signOut(data).then(userUi.onSignOutSuccess).catch(userUi.onSignOutFailure);
};
...
var onNewSongSuccess = function onNewSongSuccess(data) {
  $('.song-message').text('New song created!').show().fadeOut(2000);
  store.songs = data.songs;
  userApi.get().then(onGetSuccess);
};

var onNewSongFailure = function onNewSongFailure(data) {
  $('.prompt-div').text('Something went wrong. Please try again. All song information is required.');
};

var onGetFailure = function onGetFailure(data) {
...
var onGetSuccess = function onGetSuccess(data) {
  if (data.songs.length > 0) {
    ...
    var songHTML = songHandles({ songs: data.songs });
    $('.song-bars').text('');
    ...
    $('.delete-song').on('click', function (event) {
      ...
      userApi.deleteSong(songId).then(deleteSongSuccess).catch(deleteSongFailure);
    });
    ...
    $('.update-song').on('click', function (event) {
      ...
      $(".update-a-song").on('submit', function (event) {
        event.preventDefault();
        var songId = $(this).closest('h4').data('id');
        var data = getFormFields(this);
        userApi.patchSong(data, songId).then(updateSuccess).catch(coldCall);
      });
...
var deleteSongFailure = function deleteSongFailure() {
  $('.prompt-div').text('Action failed.');
};
var coldCall = function coldCall() {
...
module.exports = {
  ...
  onNewSongSuccess: onNewSongSuccess,
  onGetSuccess: onGetSuccess,
  deleteSongSuccess: deleteSongSuccess,
  onNewSongFailure: onNewSongFailure,
